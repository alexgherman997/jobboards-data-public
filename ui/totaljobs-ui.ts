...
export async function searchJobsTotalJobs(
  term: string,
  ...
  let searchURL = `https://www.totaljobs.com/jobs/work-from-home/${term}/in-europe?radius=20&sort=2&action=sort_publish&postedWithin=7&page=1`
  ...
  try {
    ...
    do {
      ...
      // searchURL = `https://www.totaljobs.com/jobs/work-from-home/${term}/in-europe?radius=20&sort=2&action=sort_publish&postedWithin=7&page=${j}`
    ...
    console.log('Totaljobs - Error principal function:', error.message)
    browser.close()
    return jobsArray
  }
}
...
//Debug
//searchJobsTotalJobs('test', new Map<any, any>())
